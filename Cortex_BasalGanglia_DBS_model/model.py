...
def create_network(
    ...
    if global_ctx_stimulation:
        ...
        # np.savetxt(structure_save_dir / "cortical_xz_pos.txt", Cortical_Pop.positions, delimiter=",")
...
def load_network(
    ...
    cortical_layers_space = space.RandomStructure(
        boundary=space.Sphere(1500), origin=(0, 0, -1000), rng=NumpyRNG(seed=rng_seed)
    )
    if global_ctx_stimulation:
        cortical_structure = cortical_layers_space
    else: cortical_structure = STN_space
    ...
    print(f"the pop size is: {Pop_size}")
    ...
    if global_ctx_stimulation:
        ...
        count = 0
        for idx, y in enumerate(cortical_y):
            if y != 0:
                #print(f"Non-zero Y position found at index {idx}: Y = {y}")
                count += 1
        print(f"Total cells with Y not at zero: {count} after assigning")
